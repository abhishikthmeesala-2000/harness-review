...
export const RemediationPlanSchema = z.object({
  ...
  /** Structured replacement blocks for direct file editing. */
  codeReplacements: z.array(z.object({
    file: z.string(),
...
export class RemediationAgent extends BaseAgent {
  ...
  async remediate(
    ...
    const prompt = [
      ...
      '',
      'PATCH REQUIREMENTS (mandatory for trivial/small/medium effort)',
      '',
      'suggestedPatch: valid unified diff using exact file path from diff:',
      '  --- a/<file>',
      '  +++ b/<file>',
      '  @@ -<lineStart>,<n> +<lineStart>,<m> @@',
      '  <unchanged context line>',
      '  -<exact removed line>',
      '  +<exact replacement line>',
      '  <unchanged context line>',
      'codeReplacements: one entry per changed block:',
      '  { "file": "<path>", "lineStart": N, "lineEnd": M, "replacement": "<new code>" }',
      '  lineStart/lineEnd are original file line numbers from FULL FILE CONTEXT.',
      '  Use EXACT lines from FULL FILE CONTEXT — do not paraphrase surrounding lines.',
      'large effort only: set suggestedPatch to null and explain why in plan.',
      '',
      ...
      '{ "findingId": string, "plan": string, "suggestedPatch": string | undefined, "codeReplacements": [{ "file": string, "lineStart": number, "lineEnd": number, "replacement": string }], "testRecommendations": string[], "estimatedEffort": "trivial"|"small"|"medium"|"large" }',
