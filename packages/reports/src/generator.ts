...
export const ReportGenerator = {
  generateAll(result: PipelineResult, meta: RunMetadata, config: Config): Record<string, string> {
    const out: Record<string, string> = {};
    for (const fmt of config.reports.formats) {
      if (fmt === 'json') out['json'] = JsonReport.generate(result, meta);
      else if (fmt === 'markdown') out['markdown'] = MarkdownReport.generate(result, meta);
      else if (fmt === 'html') out['html'] = HtmlReport.generate(result, meta);
    }
    return out;
  },
};
