...
export function createAlmAdapter(config: Config): AlmAdapter {
  switch (config.alm.platform) {
    case 'github': return new GitHubAlm();
    case 'gitlab': return new GitLabAlm();
    case 'azure-devops': return new AzureDevOpsAlm();
    case 'bitbucket': return new BitbucketAlm();
    default: return new NoneAlm();
  }
}
